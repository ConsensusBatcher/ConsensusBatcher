...
*/	
	...
	for(int i=0;i<players;i++){
		...
		ECP2_BN254_copy(&Sigs2[i],&M_point2);
		PAIR_BN254_G2mul(&Sigs2[i],num);
	}
...
//		share ** self.lagrange(S, j)
		...
		if(sign_FP(&larg)){
			...
			PAIR_BN254_G2mul(&tmp2,larg_num_tmp);
			ECP2_BN254_neg(&tmp);
		}else{
			FP_BN254_redc(larg_num,&larg);
			PAIR_BN254_G2mul(&tmp,larg_num);
		}
		
		ECP2_BN254_copy(&r[j],&tmp);	
	}
	
	for(int j=1 ; j<k ; j++){
		ECP2_BN254_add(&r[0],&r[j]);//������ܲ���GroupԪ���ˣ�
	}
	ECP_BN254_copy(&SIG2,&r[0]);	
...
//	}
	...
	if(sign_FP(&M_FP)){
		...
		PAIR_BN254_G1mul(&M_point2,M_num);
		ECP2_BN254_neg(&M_point2);
	}else{
		FP_BN254_redc(M_num,&M_FP);
		ECP_BN254_copy(&M_point2,&g);
		PAIR_BN254_G1mul(&M_point2,M_num);
	}
